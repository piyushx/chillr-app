import React, { useContext, useEffect, useState, useRef } from 'react'
import Editpostitem from './editpostitem'
import NoteContext from "../contextAPI/ContextAPI"

function Dashboard() {

    useEffect(() => {
        getuserdetails()
    }, [])


    const Context = useContext(NoteContext);

    const refe = useRef(null)
    const refer = useRef(null)

  

    const [Post, setPost] = useState({post: ""})

    const onChangepost = (e) => {
        setPost({ ...Post, [e.target.name]: e.target.value })
        console.log(Post);
    }

    const updatepost = async (newpost, userid) => {
        const response = await fetch(`http://localhost:5000/post/update/${userid}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                "auth-token": localStorage.getItem("authtoken")
            },
            body: JSON.stringify({ newpost:newpost })
        })
    const json = await response.json()
    let newstate = JSON.parse(JSON.stringify(userpost))

    for(let i=0; i< userpost.length ; i++) {
        const element = userpost[i];
        if (element._id === userid){
            newstate[i].user = newstate[i].user
            newstate[i].name = newstate[i].name
            newstate[i].post = newpost
            newstate[i].likes = newstate[i].likes
            newstate[i].comments = newstate[i].comments
            newstate[i].category = newstate[i].category
        }
        break;
        
    }
    setuserpost(newstate)
    }

    const functoupdate = (event) => {
        updatepost(Post.post, Post.userid)
        event.preventDefault(); 
        refer.current.click()
    }


    const openModal = (post, userid) => {
        refe.current.click()
        setPost({post, userid})
    }

    const [profile, setprofile] = useState({ name: "", bio: "", following: "" })
    const [userpost, setuserpost] = useState([])
    const [post, setpost] = useState({ post: "" })

    const postnew = async (post) => {
        const response = await fetch(`http://localhost:5000/post/new`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                "auth-token": localStorage.getItem("authtoken")
            },
            body: JSON.stringify({ post })
        });

        const json = await response.json()
        console.log(json);

        let newstate = userpost.concat(json)
        setuserpost(newstate)

    }

    const postneww = async (post) => {
        postnew(post)
        console.log(post);
    }


    const onChange = (event) => {
        setpost({ ...post, [event.target.name]: event.target.value })
        console.log(post);
    }

    const getuserdetails = async () => {
        let data = localStorage.getItem("id")
        const response = await fetch(`http://localhost:5000/auth/getuser/${data}`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                "auth-token": localStorage.getItem("authtoken")
            },
        });

        const json = await response.json()
        const userid = json._id
        console.log(json);
        setprofile({ name: json.name, bio: json.bio, following: json.following.length })

        const responses = await fetch(`http://localhost:5000/post/user/${userid}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                "auth-token": localStorage.getItem("authtoken")
            },
        });

        const jsn = await responses.json()
        console.log(jsn);
        setuserpost(jsn)
    }

    const deletepost = async (postid) => {

        const response = await fetch(`http://localhost:5000/post/delete/${postid}`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json',
                "auth-token": localStorage.getItem("authtoken")
            }
        })
    const json = await response.json()
    let allnewstate = userpost.filter((post)=> {return post._id != postid})
    setuserpost(allnewstate)
    }



    return (
        <>
            <button type="button" ref={refe} class="btn btn-primary d-none" data-bs-toggle="modal" data-bs-target="#updateModal">
                Launch demo modal
            </button>
            <div class="modal fade" id="updateModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">Update your note</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <div className="container">
                                <div class="mb-3">
                                    <label for="Title" class="form-label">Edit your post: </label>
                                    <input type="text" class="form-control" id="post" value={Post.post} onChange={onChangepost} name="post" aria-describedby="emailHelp" />
                                </div>
                            </div>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal" ref={refer}>Close</button>
                            <button type="button" class="btn btn-primary" onClick={functoupdate}>Update Post</button>
                        </div>
                    </div>
                </div>
            </div>
            <div className="container">
                <div class="card mb-3 my-5" style={{ maxWidth: "1080px" }}>
                    <div class="row g-0">
                        <div class="col-md-4">
                            <img src="data:image/png;base64," class="img-fluid rounded-start" alt="..." />
                        </div>

                        <div class="col-md-8">
                            <div class="card-body">
                                <h5 class="card-title">{profile.name}</h5>
                                <p class="card-text">BIO: {profile.bio}</p>
                                <p class="card-text"><small class="text-muted">Posts: {userpost.length}</small></p>
                                <p class="card-text"><small class="text-muted">Followers: </small></p>
                                <p class="card-text"><small class="text-muted">Following: {profile.following}</small></p>
                            </div>
                        </div>
                    </div>
                </div>

                <div class="row g-1">
                    <div class="col">
                        <input type="text" class="form-control" name="post" id="post" placeholder="Enter a comment" onChange={onChange} />
                    </div>
                    <div class="col-2">
                        <button class="btn btn-primary mb-3" onClick={() => postneww(post.post)}>Post now</button>
                    </div>
                </div>

                {userpost.map((eachpost) => <Editpostitem userid={eachpost.user} deletepost = {deletepost} openModal={openModal} username={eachpost.name} postid={eachpost._id} content={eachpost.post} likes={eachpost.likes} comments={eachpost.comments} />)}

            </div>
        </>
    )
}

export default Dashboard
